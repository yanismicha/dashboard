...
data = pd.merge(data, regions_name, left_on='dep', right_on='num_dep', how='left')
...
fig_seri_simple = px.line(accidents_par_annee, x="an", y="Nombre_d_accidents",
              markers=True,
             custom_data= ['an','Nombre_d_accidents'])
# taille de la figure (largeur, hauteur)
fig_seri_simple.update_layout(width=800, height=500)
#  ajout titre et axes
fig_seri_simple.update_layout(
                  xaxis_title="Année",
                  yaxis_title="Nombre d'accidents")
fig_seri_simple.update_layout(margin = {"r":0,"t":30,"l":0,"b":0},
                 title={
...
fig_seri_simple.update_traces(hovertemplate="<br>".join(["Année : %{customdata[0]}",
...
fig_seri_liss = px.line(accidents_par_annee_mois, x='date', y='Nombre_d_accidents',
...
fig_seri_liss.add_trace(go.Scatter(x=accidents_par_annee_mois['date'], y=smoothed_data,
                             mode='lines', name='Tendance lissée sur 12 mois'))
# Titre
fig_seri_liss.update_layout(margin = {"r":0,"t":30,"l":0,"b":0},
                     title={
...
fig_seri_liss.update_xaxes(rangeslider_visible=True)
    
# Personnalisation du popup
fig_seri_liss.update_traces(hovertemplate="<br>".join(["Année : %{customdata[0]}",
                                                 "Mois : %{customdata[1]}",
                                                 "Nombre d'accidents : %{customdata[2]}"
                                                 ]),selector= 0)
...
def pie_age_grav(modalite, annee):
    if annee=="all":
        age_mort = data[data["grav"]==modalite].groupby(["age_group"],observed=False).size().reset_index(name= "nb_accidents")
    else:
...
def density(variable, annee, data = data, title_comp=None):
    ...
    if variable != "all":
        title = title + " par " + str(variable) # Peut utiliser dico pour ajuster contenue
...
def bar(var, annee, data = data, title_comp = None):
    ...
    if var == "all":
        ...
        title = title + " par " + str(var) # Peut utiliser dico pour ajuster contenue
